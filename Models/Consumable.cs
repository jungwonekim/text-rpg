...
public class Consumable: Item
{
    ...
    public override bool Use(Player player)
    {
        ...
        if (HpAmount > 0)
        {
            int healedHp = player.HealUp(HpAmount);
            if (healedHp > 0)
            {
    ...
    {
        "체력포션" => new Consumable("체력포션", "50 HP를 회복하는 물약", 50, 50),
        "대형체력포션" => new Consumable("대형체력포션", "100HP를 회복하는 물약", 100, 100),
        "마나포션" => new Consumable("마나포션", "50 MP를 회복하는 물약", 50, 0, 50),
        "대형마나포션" => new Consumable("대형마나포션", "100MP를 회복하는 물약", 100, mpAmount: 100),
        _ => null!
    };
